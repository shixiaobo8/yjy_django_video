...
class videoForm(forms.Form):
	...
	app_type = forms.ChoiceField(choices=[('yjy_xiyizonghe','西医综合'),('tcmsq','中医执业医师'),('yjy_zhongyizonghe','中医综合'),('yjy_xiyizhiyeyishi','西医执业医师')],label='app类型',widget=forms.Select(attrs={'class':'form-control','required':True}))
	room_id = forms.IntegerField(label='聊天室id',widget=forms.TextInput(attrs={'class':'form-control','required':True}))
	name = forms.CharField(label='视频名称',widget=forms.TextInput(attrs={'class':'form-control','required':True}))
	status = forms.ChoiceField(widget=forms.RadioSelect({'required':True}),choices=STATUS_CHOICES,required=True,label='状态')
	sort = forms.ChoiceField(widget=forms.RadioSelect,choices=SORT_CHOICES,required=True,label='分类')
	zhuanorxue = forms.ChoiceField(widget=forms.CheckboxSelectMultiple(attrs={'required':True}),choices=ZHUANGXUE_CHOICES,label='专硕学硕')	
	is_on_air = forms.ChoiceField(choices=[(0,'即将开始'),(1,'正在直播'),(2,'直播已结束')],label='直播状态',widget=forms.Select(attrs={'class':'form-control','required':True}))
	goods_id = forms.IntegerField(label='商品id',widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入商品id','required':True}))
	service_id = forms.IntegerField(label='服务id',widget=forms.TextInput(attrs={'class':'form-control','placeholder':'请输入服务id','required':True}))
	media_url = forms.URLField(label='加密地址',widget=forms.URLInput(attrs={'class':'form-control','required':True}))
	download_url = forms.URLField(label='下载地址',widget=forms.URLInput(attrs={'size':20,'class':'form-control','required':True}))
	duration = forms.DurationField(label='播放时长',widget=forms.TextInput(attrs={'class':'form-control','required':True}))
	sort = forms.IntegerField(label='排序',widget=forms.TextInput(attrs={'class':'form-control','required':True}))
	parent_id = forms.ChoiceField(choices=PARENT_ID,label='科目',widget=forms.Select(attrs={'class':'form-control','onChange':'getChapters(this.value)','required':True}))
	chapter_id = forms.ChoiceField(choices=chapters,label='章节名称',widget=forms.Select(attrs={'class':'form-control','required':True}))
