...
echo > /root/shell/rs.txt
echo > /tmp/tss_file.txt
...
do
	...
	random_path=${random_path}"_"${output_name}
	...
	#切片
	file_url=${WEB_URL}/${date}/"aes_"${random_path}"/"${file_name}"_aes.m3u8"
	segmenter -y -i $file_name.ts -d 30 -p $file_name -m $file_name.m3u8  -u ${WEB_URL}/${date}/${random_path}/ &>/dev/null
	_log "生成切片成功"
	...
	#rm $tmp_file
	echo ${output_full_path},${output_name},${thumb_url},${file},${RESOLUTION},${file_url},${duration},$ts_size >> $SQL_LOG
	echo ${output_full_path}"/"${file_name}".m3u8"  >> /root/shell/rs.txt
	mu38=`cat /root/shell/rs.txt`
	for m in $mu38
	do
		cat $m | grep ts >> /tmp/tss_file.txt
	done
# 调用python脚本预热ts的url
# /usr/bin/python /root/scripts/push_new_url_to_cdn.py /tmp/tss_file.txt
done
# 处理加密视频
sh /root/scripts/get_aes.sh
